...
class CostEstimate:
    ...
    def add_component(self, name: str, cost: float, unit: str = "month", details: str = ""):
        ...
        self.breakdown.append({"component": name, "cost": round(cost, 2), "unit": unit, "details": details})
...
async def estimate_azure_vm_cost_realtime(parameters: Dict[str, Any]) -> CostEstimate:
    ...
    try:
        ...
        vm_pricing = await azure_client.get_vm_pricing(vm_size=vm_size, region=region, operating_system=os_type)
        ...
        disk_pricing = await azure_client.get_disk_pricing(disk_type="StandardSSD_LRS", region=region)

        if disk_pricing:
            disk_cost = disk_size_gb * (disk_pricing["price_per_month"] / 32)  # Approximate per GB
            estimate.add_component("OS Disk", disk_cost, "month", f"{disk_size_gb} GB Standard SSD")
        else:
            # Fallback
...
def estimate_azure_vm_cost_fallback(parameters: Dict[str, Any]) -> CostEstimate:
    ...
    if vm_cost_info:
        estimate.add_component(
            "Virtual Machine", vm_cost_info["cost_per_month"], "month", f"{vm_size} - {vm_cost_info['description']}"
        )
    else:
        estimate.add_component("Virtual Machine", 50.0, "month", f"{vm_size} (estimated)")
...
async def estimate_gcp_vm_cost_realtime(parameters: Dict[str, Any]) -> CostEstimate:
    ...
    try:
        ...
        compute_pricing = await gcp_client.get_compute_pricing(machine_type=machine_type, region=region)
        ...
        # Fetch disk pricing
        disk_pricing = await gcp_client.get_disk_pricing(disk_type=boot_disk_type, region=region)

        if disk_pricing:
            disk_cost = boot_disk_size * disk_pricing["price_per_gb_month"]
            estimate.add_component("Boot Disk", disk_cost, "month", f"{boot_disk_size} GB {boot_disk_type}")
        else:
            # Fallback
...
async def estimate_azure_storage_cost_realtime(parameters: Dict[str, Any]) -> CostEstimate:
    ...
    try:
        ...
        storage_pricing = await azure_client.get_storage_pricing(
            storage_type=tier, region=region, redundancy=replication_type
        )
...
def estimate_azure_storage_cost_fallback(parameters: Dict[str, Any]) -> CostEstimate:
    ...
    estimate.add_component(
        "Storage Cost", storage_cost, "month", f"{estimated_gb} GB - {storage_cost_info['description']}"
    )
...
async def estimate_deployment_cost(
    template_name: str, provider_type: str, parameters: Dict[str, Any]
) -> Dict[str, Any]:
    """
...
def estimate_deployment_cost_sync(template_name: str, provider_type: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
    """
