...
from backend.core.database import CloudAccount, SessionLocal, UserCloudPermission
...
@router.get("", summary="List Cloud Accounts", response_model=StandardResponse)
async def list_cloud_accounts(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        if is_admin:
            ...
            permissions = (
                db.query(UserCloudPermission).filter(UserCloudPermission.user_email == current_user["email"]).all()
            )
            account_ids = [p.cloud_account_id for p in permissions]
            accounts = db.query(CloudAccount).filter(CloudAccount.id.in_(account_ids)).all() if account_ids else []

        return success_response(
            message=f"Retrieved {len(accounts)} cloud accounts",
            data={"accounts": [acc.to_dict(include_secrets=False) for acc in accounts], "total": len(accounts)},
        )
    finally:
        db.close()


@router.post("", summary="Create Cloud Account", response_model=StandardResponse, status_code=status.HTTP_201_CREATED)
async def create_cloud_account(account_data: CloudAccountCreate, current_user: dict = Depends(get_current_user)):
...
@router.get("/user/permissions", summary="Get Current User's Cloud Permissions", response_model=StandardResponse)
async def get_user_permissions(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        if is_admin:
            ...
            permissions_data = [
                {"account": acc.to_dict(include_secrets=False), "can_deploy": True, "can_view": True}
                for acc in accounts
            ]
        else:
            ...
            for perm in permissions:
                account = (
                    ...
                    .filter(CloudAccount.id == perm.cloud_account_id, CloudAccount.is_active == "true")
                    .first()
                )
                if account:
        ...
        return success_response(
            ...
            data={"permissions": permissions_data, "is_admin": is_admin},
        )
    finally:
        db.close()
...
@router.get("/{account_id}", summary="Get Cloud Account", response_model=StandardResponse)
async def get_cloud_account(account_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        return success_response(
            message="Cloud account retrieved", data={"account": account.to_dict(include_secrets=is_admin)}
        )
    finally:
        db.close()


@router.put("/{account_id}", summary="Update Cloud Account", response_model=StandardResponse)
async def update_cloud_account(
    account_id: str, update_data: CloudAccountUpdate, current_user: dict = Depends(get_current_user)
):
...
@router.delete("/{account_id}", summary="Delete Cloud Account", response_model=StandardResponse)
async def delete_cloud_account(account_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        db.query(UserCloudPermission).filter(UserCloudPermission.cloud_account_id == account_id).delete()
        ...
        return success_response(
            message=f"Cloud account '{account_name}' deleted successfully", data={"deleted_id": account_id}
        )
    except HTTPException:
        raise
    except Exception as e:
...
@router.get("/{account_id}/permissions", summary="List Account Permissions", response_model=StandardResponse)
async def list_account_permissions(account_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        permissions = db.query(UserCloudPermission).filter(UserCloudPermission.cloud_account_id == account_id).all()

        return success_response(
            message=f"Retrieved {len(permissions)} permissions",
            data={"permissions": [p.to_dict() for p in permissions], "total": len(permissions)},
        )
    finally:
        db.close()
...
async def assign_permission(
    account_id: str, permission_data: PermissionCreate, current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        return success_response(
            message=f"Permission assigned to {permission_data.user_email}", data={"permission": permission.to_dict()}
        )
    except HTTPException:
        raise
    except Exception as e:
...
@router.put("/{account_id}/permissions/{user_email}", summary="Update Permission", response_model=StandardResponse)
async def update_permission(
    account_id: str, user_email: str, update_data: PermissionUpdate, current_user: dict = Depends(get_current_user)
):
    ...
    try:
        permission = (
            db.query(UserCloudPermission)
            .filter(UserCloudPermission.user_email == user_email, UserCloudPermission.cloud_account_id == account_id)
            .first()
        )
        ...
        return success_response(
            message=f"Permission updated for {user_email}", data={"permission": permission.to_dict()}
        )
    except HTTPException:
        raise
    except Exception as e:
...
@router.delete("/{account_id}/permissions/{user_email}", summary="Remove Permission", response_model=StandardResponse)
async def remove_permission(account_id: str, user_email: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        return success_response(message=f"Permission removed for {user_email}", data={"removed_email": user_email})
    except HTTPException:
        raise
    except Exception as e:
        db.rollback()
