...
async def get_azure_vm_sizes(location: str = Query(..., description="Azure location")):
    ...
    try:
        ...
        return success_response(
            f"Available VM sizes for {location}", {"location": location, "vm_sizes": vm_sizes, "count": len(vm_sizes)}
        )
...
async def get_azure_locations():
    ...
    if not _has_valid_credentials():
        return success_response(
            "Available Azure locations (fallback)", {"locations": FALLBACK_LOCATIONS, "count": len(FALLBACK_LOCATIONS)}
        )
    ...
    try:
        ...
        return success_response("Available Azure locations", {"locations": locations, "count": len(locations)})
...
async def get_azure_resource_groups():
    ...
    if not _has_valid_credentials():
        return success_response(
            "Azure credentials not configured", {"resource_groups": [], "count": 0, "can_create_new": True}
        )
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers={"Authorization": f"Bearer {token.token}"}) as response:
                if response.status == 200:
                    ...
                    resource_groups = [
                        {"name": rg.get("name"), "location": rg.get("location")} for rg in data.get("value", [])
                    ]
                    return success_response(
        ...
        return success_response(
            "Could not fetch resource groups", {"resource_groups": [], "count": 0, "can_create_new": True}
        )

    except Exception as e:
        logger.warning(f"Error fetching Azure resource groups: {e}")
        return success_response(
            "Error fetching resource groups", {"resource_groups": [], "count": 0, "can_create_new": True}
        )
