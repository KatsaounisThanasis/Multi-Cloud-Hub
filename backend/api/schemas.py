...
def error_response(message: str, details: Any = None, status_code: int = 500) -> JSONResponse:
    """
    ...
    """
    response = StandardResponse(success=False, message=message, error={"details": details, "status_code": status_code})
...
class DeploymentRequest(BaseModel):
    ...
    def validate_resource_group(cls, v: str) -> str:
        ...
        if not re.match(r"^[\w\-\.\(\)]+$", v):
            raise ValueError(
                "Resource group can only contain alphanumerics, underscores, hyphens, periods, parentheses"
            )
        return v
    ...
    def validate_parameters(cls, v: Dict[str, Any]) -> Dict[str, Any]:
        ...
        from backend.utils.validators import InvalidParameterError, validate_app_name
