...
class TestAuthRegister:
    ...
    def test_register_success(self, client):
        ...
        with patch("backend.api.routers.auth.create_user") as mock_create:
            ...
            response = client.post(
                "/auth/register", json={"email": "new@example.com", "password": "password123", "username": "newuser"}
            )
    ...
    def test_register_duplicate_email(self, client):
        ...
        with patch("backend.api.routers.auth.create_user") as mock_create:
            ...
            response = client.post(
                "/auth/register",
                json={"email": "existing@example.com", "password": "password123", "username": "newuser"},
            )
    ...
    def test_register_invalid_email(self, client):
        ...
        response = client.post(
            "/auth/register", json={"email": "invalid-email", "password": "password123", "username": "newuser"}
        )
    ...
    def test_register_short_password(self, client):
        ...
        response = client.post(
            "/auth/register", json={"email": "test@example.com", "password": "123", "username": "newuser"}
        )
...
class TestUserManagement:
    ...
    def test_update_user_self(self, authenticated_client, mock_user):
        ...
        with patch("backend.api.routers.auth.update_user") as mock_update:
            ...
            response = authenticated_client.put(f"/auth/users/{mock_user['email']}", json={"username": "updateduser"})

            assert response.status_code == 200

    def test_update_user_other_as_non_admin(self, authenticated_client):
        """Test updating another user as non-admin (should fail)."""
        response = authenticated_client.put("/auth/users/other@example.com", json={"username": "hacker"})
        assert response.status_code == 403

    def test_update_role_as_non_admin(self, authenticated_client, mock_user):
        """Test changing role as non-admin (should fail)."""
        response = authenticated_client.put(f"/auth/users/{mock_user['email']}", json={"role": "admin"})
        assert response.status_code == 403
